...
def hom_space_frobenius_error(
    coefficients: Tensor,
    reference_coefficients: Tensor,
    basis_norm_sq: Tensor,
) -> Tensor:
    """Return ``||A - B||_F^2`` from orthogonal homomorphism-basis coefficients."""

    delta = coefficients - reference_coefficients
    return torch.sum(delta.pow(2) * basis_norm_sq, dim=-1)
...
def symm_orthonormality_regularization(
    ...
    """Compute ``||V_x - I||_F^2`` directly in the free degrees of freedom of ``Hom_G``."""

    Cx_I_err_fro_2 = hom_space_frobenius_error(
        coefficients=Cx_dof,
        reference_coefficients=identity_dof,
        basis_norm_sq=basis_norm_sq,
    )
    x_centering_loss = (x_mean**2).sum()
    orthonormality_x = Cx_I_err_fro_2 + 2 * x_centering_loss

    with torch.no_grad():
        metrics = {
            f"||mu_{var_name}||": (torch.sqrt(x_centering_loss) / ambient_dim).item(),
            f"||V{var_name} - I||_F^2": (orthonormality_x / ambient_dim).item(),
        }
