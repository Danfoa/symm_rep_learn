...
def plot_analytic_joint_2D(
    ...
    for key, cpd_vals, expect_y, Pey in zip(
        ["cpd_x", "cpd_gx"], [cpd_y_vals, cpd_gy_vals], [E_y, E_gy], [P_E_y, P_E_gy]
    ):
        grid.ax_marg_y.plot(
            cpd_vals,
            y_range,
            color=PLOT_STYLE[key]["color"],
            linestyle="-",
            alpha=0.9,
...
def plot_analytic_prod_2D(
    ...
    # Select a random sample to test the conditional expectation
    np.random.choice(len(x_samples))
    x_t, y_t = -1, 1
...
def plot_analytic_npmi_2D(
    ...
    X_input = np.column_stack([X_flat])
    Y_input = np.column_stack([Y_flat])
    gmm.joint_pdf(X=X_input, Y=Y_input)
...
def plot_pmd_err_2D(
    ...
    # Plot constant line at 1 for both marginals
    grid.ax_marg_y.axvline(
        1,
        color="k",
        linestyle="-",
        alpha=0.2,
    )
    grid.ax_marg_x.axhline(
        1,
        color="k",
        linestyle="-",
        alpha=0.2,
    ...
    for key, pmd_x, pmd_x_pred in zip(
        ["npmi_x", "npmi_gx"], [pmd_x_vals, pmd_gx_vals], [pmd_x_vals_pred, pmd_gx_vals_pred]
    ):
        grid.ax_marg_y.plot(
            pmd_x,
            y_range,
    ...
    for key, pmd_y, pmd_y_pred in zip(
        ["npmi_y", "npmi_gy"], [pmd_y_vals, pmf_gy_vals], [pmd_y_vals_pred, pmf_gy_vals_pred]
    ):
        grid.ax_marg_x.plot(
            x_range,
            pmd_y,
