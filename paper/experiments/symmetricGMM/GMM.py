...
class GaussianMixture(BaseConditionalDensitySimulation):
    ...
    def __init__(self, n_kernels=5, ndim_x=1, ndim_y=1, means_std=1.5, random_seed=None):
        ...
        self.covariances_x = project_to_pos_semi_def(
            np.abs(self.random_state_params.normal(loc=1, scale=0.5, size=(n_kernels, self.ndim_x, self.ndim_x)))
        )  # shape(n_kernels, ndim_x, ndim_y)
        self.covariances_y = project_to_pos_semi_def(
            np.abs(self.random_state_params.normal(loc=1, scale=0.5, size=(n_kernels, self.ndim_y, self.ndim_y)))
        )  # shape(n_kernels, ndim_x, ndim_y)
    ...
    def _W_x(self, X):
        ...
        if X.shape[0] == 1:
            w_p = np.stack(
                [np.array([self.weights[i] * self.gaussians_x[i].pdf(X)]) for i in range(self.n_kernels)],
                axis=1,
            )
        else:
            w_p = np.stack(
                [self.weights[i] * self.gaussians_x[i].pdf(X) for i in range(self.n_kernels)],
                axis=1,
            )
