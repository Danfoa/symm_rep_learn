...
from symm_rep_learn.models.multivariateCQR import (
    EquivMultivariateCQR,
    MultivariateCQR,
    get_coverage,
...
def get_uc_model(cfg: DictConfig, x_type, y_type) -> torch.nn.Module:
    ...
    if cfg.model.lower() == "cqr":  # Conditional Quantile Regression
        ...
        equiv_cqr = EquivMultivariateCQR(
            in_type=x_type,
            out_type=y_type,
            gamma=cfg.alpha,
            **nn_kwargs,
...
def main(cfg: DictConfig):
    ...
    is_equiv_model = isinstance(model, ENCP) or isinstance(model, EMLP) or isinstance(model, EquivMultivariateCQR)
    dl_kwargs = dict(
        ds_cfg=cfg.dataset,
        x_moments=x_moments,
        y_moments=y_moments,
    ...
    if isinstance(model, MultivariateCQR) or isinstance(model, EquivMultivariateCQR):
