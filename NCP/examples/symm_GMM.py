...
@torch.no_grad()
def measure_analytic_mi_error(gmm, ncp, x_samples, y_samples, x_type, y_type, x_mean, y_mean, x_var, y_var,
                              plot=False, samples=2048):
    G = x_type.fibergroup
    ...
    mi_mse_pos = ((np.maximum(0, mi_xy.cpu().numpy()) - mi_xy_gt) ** 2).mean()
    mi_mse_trivial = ((mi_xy_gt - 1) ** 2).mean()
    # Since k(x, y) = k(g.x, g.y) for all g in G, we want to compute the variance of the estimate under g-action
    mi_xy_g_var = mi_xy.view(G.order(), -1).var(dim=0)  # Var[ k_r(g.x, g.y) ] for all g in G
    ...
    metrics = {"MI/mse":         mi_mse,
               "MI/mse_pos":     mi_mse_pos,
               "MI/mse_trivial": mi_mse_trivial,
    ...
    if plot:
        ...
        for i, (cdf, cdf_gt) in enumerate(zip(CDFs, CDFs_gt)):
            ...
            fig.add_trace(go.Scatter(x=Y_range_np.squeeze(), y=cdf.squeeze(), mode='lines', name=r"$\hat{p}(y | x)$",
