...
PYTHONPATH=. python ${exec_file} ${hydra_kwargs} ${exp_params} device=3 model=ENCP architecture.residual_encoder=False lstsq=False gamma=10     seed=0,1,2 &
PYTHONPATH=. python ${exec_file} ${hydra_kwargs} ${exp_params} device=4 model=ENCP architecture.residual_encoder=False lstsq=False gamma=25     seed=0,1,2 &
